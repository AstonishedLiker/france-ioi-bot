...
def scrape_levels_chapters(account: Account, html_doc: str) -> Optional[list[Level]]:
    ...
    for child in levelTableBody.find_all("tr"):
        ...
        if td["class"][0] == "chapters-category":
            levelHeading = td.h2
            assert levelHeading is not None
        ...
        elif currentLevel and not currentLevel.locked: # So we don't add locked chapters, since we can't see them yet.
            anchor = child.find("a")
            ...
            assert currentLevel is not None
